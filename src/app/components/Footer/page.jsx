import Image from "next/image";
import React from "react";
import logo from "../../../../public/logo.png";
import { MapPin, Phone, ChevronRight } from "lucide-react";
import { Button } from "@heroui/react";

const Footer = () => {
  return (
    <div className="p-[5%] w-full blue_bg">
      <div className="flex flex-col justify-center items-center w-full p-[2%] bg-gray-500">
        <div className="grid grid-cols-12 gap-5">
          <div className="col-span-4">
            <h1 className="sub_blue">Business Hub</h1>
            <p>
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Facilis
              soluta eius maxime id, error neque quibusdam blanditiis atque
            </p>
            <div className="flex justify-start items-center gap-2 my-3">
              <MapPin />
              <p>Location: D-Ground, Satiyana Road</p>
            </div>
            <div className="flex justify-start items-center gap-2 my-3">
              <Phone />
              <p>Location: D-Ground, Satiyana Road</p>
            </div>
          </div>
          <div className="col-span-2 space-y-3">
            <span className="text-2xl font-bold yellow">Quick Links</span>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Home</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>About</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Services</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Contact</p>
            </div>
          </div>
          <div className="col-span-2 space-y-3">
            <span className="text-2xl font-bold yellow">Useful Links</span>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Privacy Policy</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Terms and Conditions</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Disclaimer</p>
            </div>
            <div className="flex justify-start items-center">
              <ChevronRight />
              <p>Support</p>
            </div>
          </div>
          <div className="col-span-4">
            <span className="yellow text-2xl font-bold">Newsletter</span>
            <p className="my-2">
              Lorem ipsum dolor, sit amet consectetur adipisicing elit.
              Perspiciatis, rerum accusamus.
            </p>
            <form className="my-5 flex flex-col gap-3" action="">
              <input type="text" className="border border-black w-80 py-2 rounded" placeholder=" Your Email..." />
              <Button className="w-80 py-2">
                Subscribe
              </Button>
            </form>
          </div>
        </div>
        <div></div>
      </div>
    </div>
  );
};

export default Footer;
